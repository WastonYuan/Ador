...
func (r *Record) Read(txn *TXN) int {
	for batch_i := batch_index(txn.txn_id); batch_i >= 0; batch_i-- {
		batch_wts := r.GetBatchWts(batch_i)
		wts := r.GetWts()
		if batch_wts < wts { // this happen when access the samller batch (the wts's batch is overwrite by a larger batch_wts than wts)
			return wts
		} else if batch_wts != -1 && batch_wts < txn.txn_id {
			return batch_wts
		}
	}
	return -1
