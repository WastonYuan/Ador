...
func NewVersion(wts int, count int, stats int) *Version {
	return &Version{wts, count, stats, &(sync.RWMutex{})}
}
...
func (v *Version) Write() (int, bool) {
	...
	if v.count != 0 {
		...
		v.stats = COMMITED // can note this to change to read committed
	}
	return 1, true
}